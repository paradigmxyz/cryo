...
// ) { print_header("cryo parameters"); let datatype_strs: Vec<_> = query.schemas.keys().map(|d|
//   d.dataset().name()).collect(); print_bullet("datatypes", datatype_strs.join(", "));
//   print_bullet("network", &sink.prefix); // let rpc_url = cli::parse_rpc_url(args); //
//   print_bullet("provider", rpc_url); print_block_chunks(query); print_transaction_chunks(query);
//   print_bullet( "chunks to collect", format!( "{} / {}",
//   n_chunks_remaining.separate_with_commas(), query.chunks.len().separate_with_commas() ), );
//   print_bullet("max concurrent chunks", source.max_concurrent_chunks.separate_with_commas()); if
//   query.schemas.contains_key(&Datatype::Logs) { print_bullet("inner request size",
//   source.inner_request_size.to_string()); }; print_bullet("output format", sink.format.as_str());
//   print_bullet("output dir", sink.output_dir.to_string_lossy()); match report_path { None =>
//   print_bullet("report file", "None"), Some(path) => print_bullet("report file", path), };
...
// ) { let dt_start: DateTime<Local> = t_start.into(); let dt_data_done: DateTime<Local> =
//   t_data_done.into();
...
//     freeze_summary: &FreezeSummary,
//     total_time: f64,
// ) { let transaction_chunks: Vec<TransactionChunk> = query .chunks .iter() .filter_map(|x| match x
//   { (Chunk::Transaction(chunk), _) => Some(chunk.clone()), _ => None, }) .collect(); if
//   !transaction_chunks.is_empty() { // let total_transactions = transaction_chunks.size() as f64;
//   let total_transactions = transaction_chunks .iter() .map(|chunk| match chunk {
//   TransactionChunk::Values(values) => values.len() as u64, TransactionChunk::Range(_, _) =>
//   todo!(), }) .sum::<u64>(); let transactions_completed: u64 = total_transactions *
//   (freeze_summary.n_completed / transaction_chunks.len() as u64); print_bullet("transactions
//   collected", transactions_completed.separate_with_commas()); let transactions_per_second =
//   transactions_completed as f64 / total_time; let transactions_per_minute =
//   transactions_per_second * 60.0; let transactions_per_hour = transactions_per_minute * 60.0; let
//   transactions_per_day = transactions_per_hour * 24.0; print_bullet("transactions per second",
//   format_float(transactions_per_second)); print_bullet("transactions per minute",
//   format_float(transactions_per_minute)); print_bullet( "transactions per hour", "  ".to_string()
//   + format_float(transactions_per_hour).as_str(), ); print_bullet( "transactions per day", "
