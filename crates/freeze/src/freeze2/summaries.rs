...
//     FileOutput, FreezeSummary, MetaChunk, Query, Source, Table,
// };
...
//     let chunks: Vec<&MetaChunk> = query.chunks.iter().map(|(label, chunk)| chunk).collect();
//     print_chunks(chunks);
//     print_bullet(
//         "chunks to collect",
//         format!(
...
// fn print_chunks(chunks: Vec<&MetaChunk>) {
...
//             print_bullet("min", min_value_string);
//         };
//         if let Some(max_value_string) = dim_stats.max_value_to_string() {
//             print_bullet("max", max_value_string);
//         };
//         print_bullet("n_values", dim_stats.total_values.to_string());
//         print_bullet("n_chunks", dim_stats.n_chunks.to_string());
//         print_bullet("chunk size", dim_stats.chunk_size.to_string());
//     }
// }

// // fn print_block_chunks(chunks: &Vec<MetaChunk>) {
// //     let mut min = None;
// //     let mut max = None;
...
//     print_bullet(
//         "chunks skipped",
//         format!("  {} / {}", freeze_summary.skipping.len().separate_with_commas(), n_chunks),
//     );
//     print_bullet(
//         "chunks collected",
...
//     let chunks: Vec<MetaChunk> = query.chunks.iter().map(|(label, chunk)| chunk.clone()).collect();
//     print_chunks_speeds(chunks, total_time);
// }

// fn print_chunks_speeds(chunks: Vec<MetaChunk>, total_time: f64) {
//     print_chunk_speed("blocks", total_time, chunks.iter().map(|c| c.block_numbers).collect());
//     print_chunk_speed("blocks", total_time, chunks.iter().map(|c| c.block_ranges).collect());
//     print_chunk_speed("transactions", total_time, chunks.iter().map(|c| c.block_ranges).collect());
//     print_chunk_speed("call_data", total_time, chunks.iter().map(|c| c.block_ranges).collect());
// }

// fn print_chunk_speed<T: ChunkData>(name: &str, total_time: f64, chunks: Vec<Option<Vec<T>>>) {
//     let flat_chunks: Vec<T> = chunks.into_iter().filter_map(|x| x).flatten().collect();
...
//     print_bullet(name + " per second", format_float(per_second));
//     print_bullet(name + " per minute", format_float(per_minute));
//     print_bullet(name + " per hour", "  ".to_string() + format_float(per_hour).as_str());
...
// // fn print_transaction_chunk_summary(query: &Query, freeze_summary: &FreezeSummary, total_time: f64) {
// //     let transaction_chunks: Vec<TransactionChunk> = query
// //         .chunks
// //         .iter()
// //         .filter_map(|x| match x {
