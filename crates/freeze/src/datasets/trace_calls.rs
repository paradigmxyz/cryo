...
fn process_action(action: &Action, columns: &mut TraceCallColumns, schema: &Table) {
    match action {
        Action::Call(action) => {
...
fn process_result(result: &Option<Res>, columns: &mut TraceCallColumns, schema: &Table) {
    match result {
